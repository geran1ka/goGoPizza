...
export const renderCart = () => {
  ...
  console.log("cartPriceElem: ", cartPriceElem);
  const contimueBtn = document.querySelectorAll(".modal-cart__continue");
  const nextBtn = document.querySelectorAll(".modal-cart__prev");
  ...
  const renderCartList = async () => {
    ...
    if (cartControl.cartData.length) {
      ...
      const cartsData = await Promise.all(
        cartControl.cartData.map(
          async (item) => await getData(`${URL_API}/api/products/${item.id}`)
        )
      );

      const cardsCart = cartsData.map((data, i) => {
    ...
    cartPriceElem.textContent = `${totalPrice} ₽`;
...
//   <source
//     srcset="
//       http://go-go-pizza-api.onrender.com/img/pepperoni.webp
//     "
//     type="image/webp"
//   />
//   <img
//     class="modal-cart__image"
//     src="img/pizza-hawaiana.png"
//     width="63"
//     height="63"
...
//       <p class="modal-cart__price">1480 ₽</p>
